...
export class AdminController {
  ...
  @Get('quote-background')
  quoteBackgrounds() {
    return this.quoteBackgroundSettings();
  }
  ...
  async createQuoteBackground(
    ...
    const next: QuoteBackgroundSettings = [
      {
        id: randomUUID(),
        imageUrl,
    ...
    await this.saveQuoteBackgroundSettings(next);
    return next[0];
  }
  ...
  async updateQuoteBackground(
    ...
    if (index < 0) throw new BadRequestException('Không tìm thấy ảnh nền.');
  ...
  @Delete('quote-background/:id')
  async deleteQuoteBackground(@Param('id') id: string) {
    const items = await this.quoteBackgroundSettings();
    const deleted = items.find((item) => item.id === id);
    if (!deleted) throw new BadRequestException('Không tìm thấy ảnh nền.');
    await this.saveQuoteBackgroundSettings(items.filter((item) => item.id !== id));
    await this.r2.deletePublicUrl(deleted.imageUrl);
    return deleted;
  }
  ...
  private async saveQuoteBackgroundSettings(items: QuoteBackgroundSettings) {
    await this.prisma.appSetting.upsert({
      where: { key: quoteBackgroundKey },
